...
const Formulario = ({ setPacientes, pacientes, paciente, setPaciente }) => {
  ...
  //   cada vez k se ejecuta el paciente
  useEffect(() => {
  ...
  const handleSubmit = (e) => {
    ...
    if ([nombre, propietario, email, alta, sintoma].includes("")) {
      console.log("todos los campos son obliga");
      setError(true);
      return;
    }
    ...
    if (paciente.id) {
      ...
      const pacienteActua = pacientes.map((pacienteState) =>
        pacienteState.id === paciente.id ? objPaciente : pacienteState
      );
      setPacientes(pacienteActua);
      setPaciente({});
    } else {
      // creando paciente
      ...
      >
        ...
        <div className="mb-5">
          ...
          <input
            id="email"
            type="email"
            placeholder="E-mail Contanto Propietario"
            className="border-2 w-full p-2 mt-2 placeholder-zinc-400 rounded-md"
            value={email}
